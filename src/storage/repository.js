...
export const exportAll = () => {
  root.getDatabase((repositories, questions) => {
    ...
    let dataBase = convertRepositoriesToExport(
      repositories,
      headerRepositories
    );
...
const convertRepositoriesToExport = (repositories, header) => {
  ...
  for (const i in repositories) {
    cvsFile += processRepository(repositories[i]);
  }
  return cvsFile;
};
...
const convertQuestionsToExport = (questions, header) => {
  ...
  for (const i in questions) {
    cvsFile += processQuestion(questions[i]);
  }
  return cvsFile;
};

const processQuestion = (q) => {
  return `${f(q.id)};${f(q.value)};${f(q.number)};${f(q.attributes.type)};${f(
    q.attributes.marker
  )};${f(q.attributes.time)};${f(q.repositoryID)}\n`;
};
